...
- The UI "doesn't look professional enough" but the reason is unclear
- You've received usability or experience-related feedback
- Pre-launch UI quality optimization
- Cross-platform design alignment is needed (Web / iOS / Android)
...
- Work limited to API or database design
- Performance optimization unrelated to the interface
- Infrastructure or DevOps work
- Non-visual scripts or automation tasks

**Rule of thumb**: if the task changes how something **looks, feels to use, moves, or is interacted with**, this Skill should be used.

## Rule Categories by Priority

*For human/AI reference: use 1→10 to decide which rule category to focus on first; query with `--domain <Domain>` for details. The script itself does not read this table.*
...
- **Stack**: React (web) — this project's tech stack
...
Scope notice: The rules below were originally written for App UI (iOS/Android/React Native/Flutter). Touch-target sizes, safe-area, and haptic-feedback items are mobile-specific — for a React website, prioritize the hover/cursor, keyboard-nav, and responsive-breakpoint guidance in the Quick Reference section above instead.

### Icons & Visual Elements

- Default icon library is **Phosphor (`@phosphor-icons/react`)**. The icons listed in `src/ui-ux-pro-max/data/icons.csv` are only commonly-recommended icons, not the complete set.
- When a suitable icon isn't found in the recommendation table:
  - **Prefer continuing to select any semantically closer icon from Phosphor's full icon set**;
  - If Phosphor also has no ideal option, you may use **Heroicons (`@heroicons/react`)** as a fallback, taking care to keep the style consistent (linear/filled, stroke weight, corner-rounding style).
