        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[var(--text-main)] tracking-tight leading-none">
               Welcome back, {user?.full_name || 'Sales Agent'}
            </h1>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch animate-entrance-up pb-2" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="workspace-card p-6 h-full flex flex-col lg:col-span-1">
          <h3 className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.1em] mb-5 flex items-center gap-2">
            <Users size={14} className="text-[var(--accent)]" /> Recent Customers (My Creations)
          </h3>
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {stats?.recentCustomers && stats.recentCustomers.length > 0 ? (
              <div className="space-y-3">
                {stats.recentCustomers.map(c => (
                  <div key={c.customer_id} className="p-3 border border-[var(--border-color)] rounded-xl bg-[var(--bg-workspace)]">
                    <div className="font-bold text-[13px] text-[var(--text-main)]">{c.customer_name}</div>
                    <div className="text-[11px] text-[var(--text-muted)]">{c.company_name}</div>
                    <div className="text-[10px] text-[var(--text-dim)] mt-1">{format(new Date(c.created_at), 'MMM d, yyyy')}</div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-[13px] text-[var(--text-muted)] italic text-center mt-10">No recent customers found.</div>
            )}
          </div>
        </div>
        ...
        <div className="workspace-card p-6 h-full flex flex-col lg:col-span-1">
          <h3 className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.1em] mb-5 flex items-center gap-2">
            <ShoppingBag size={14} className="text-[var(--accent)]" /> Recent Sales Requests
          </h3>
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {stats?.recentSales && stats.recentSales.length > 0 ? (
              <div className="space-y-3">
                {stats.recentSales.map(s => (
                  <div key={s.id} className="p-3 border border-[var(--border-color)] rounded-xl bg-[var(--bg-workspace)]">
                    <div className="font-bold text-[13px] text-[var(--text-main)]">{s.product_name} x {s.quantity}</div>
                    <div className="text-[11px] text-[var(--text-muted)]">To: {s.customer_name}</div>
                    <div className="text-[10px] text-[var(--text-dim)] mt-1">{format(new Date(s.created_at), 'MMM d, yyyy')}</div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-[13px] text-[var(--text-muted)] italic text-center mt-10">No recent sales found.</div>
            )}
          </div>
        </div>
