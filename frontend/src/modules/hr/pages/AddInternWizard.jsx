...
import { createInternApi } from '../../../api/intern';
...
const AddInternWizard = () => {
  ...
  const [formData, setFormData] = useState(() => {
    const draft = localStorage.getItem('intern_wizard_draft');
    if (draft) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      localStorage.setItem('intern_wizard_draft', JSON.stringify({ formData: draftData, currentStep }));
    }, 500);
  ...
  const handleClearDraft = () => {
    localStorage.removeItem('intern_wizard_draft');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createInternApi(formData);
      if (res.data?.success) {
          if (candidateId) {
            try {
            ...
            try {
              // Automatically create onboarding record for intern
              await createOnboardingRecordApi({
                  intern_id: res.data.data?.intern_id || res.data.intern_id,
          ...
          localStorage.removeItem('intern_wizard_draft');
          navigate(candidateId ? '/hr/onboarding' : '/hr/intern');
      }
    } catch (error) {
    ...
    <div className="max-w-[1600px] mx-auto pb-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-4 mt-2">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/hr/intern')} className="p-2.5 hover:bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl transition-colors bg-[var(--bg-workspace)] shadow-sm">
            <ArrowLeft size={20} className="text-[var(--text-main)]" />
          </button>
          <div>
            <h1 className="text-3xl font-black text-[var(--text-main)] tracking-tight">Add Intern</h1>
            <p className="text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider mt-1">
              Add a new intern to the workspace
            </p>
          </div>
        </div>
        ...
        <form id="wizard-form" onSubmit={handleSubmit} className="p-6 lg:p-8 flex-1 bg-[var(--bg-workspace)]">
          ...
          <div className="mt-10 pt-6 border-t border-[var(--border-color)] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button 
                type="button" 
                onClick={() => { handleClearDraft(); navigate('/hr/intern'); }}
            ...
            {currentStep < STEPS.length ? (
              ...
              >
                ...
                {isSubmitting ? 'Creating Intern...' : 'Confirm & Create Intern'}
              </button>
            )}
          </div>
        </form>
...
export default AddInternWizard;
