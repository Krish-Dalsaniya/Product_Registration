...
const HRUserAccessPage = () => {
  ...
  // Use employees directly, filtering out 'Admin' role users
  const baseUsers = employees.filter(e => e.role_name !== 'Admin');
  
  const uniqueRoles = [...new Set(baseUsers.map(u => u.role_name).filter(Boolean))];
  ...
  const handleViewAccess = (user) => {
    navigate(`/hr/user-access/${user.user_id}?mode=view`);
  };

  const handleEditAccess = (user) => {
    navigate(`/hr/user-access/${user.user_id}?mode=edit`);
  };
    ...
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-[var(--text-main)] capitalize flex items-center gap-2">
              {row.full_name}
              {!row.is_active && (
                <span className="text-[9px] font-black uppercase tracking-widest text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded ml-2">
                  Suspended / Deleted
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--text-muted)] font-bold mt-2 uppercase tracking-[0.2em] opacity-70">
              Manage permissions for employees in HR
            </p>
          </div>
        </div>
      </div>
...
export default HRUserAccessPage;
