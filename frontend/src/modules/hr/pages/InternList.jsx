...
const InternList = () => {
    ...
    const [stats, setStats] = useState({ total_interns: 0, active_interns: 0, completed_training: 0, converted_to_employees: 0 });
    ...
    const [convertingIntern, setConvertingIntern] = useState(null);
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const [internsRes, statsRes, metaRes, empRes, lmsRes] = await Promise.all([
                fetchInternsApi(),
                fetchInternDashboardStatsApi(),
                fetchHRMetadataApi(),
                fetchHREmployeesApi(),
                getAllModulesApi()
            ]);
    ...
    const handleOpenModal = (intern = null) => {
        if (intern) {
            ...
            setCroppedImageUrl(intern.image_url || null);
            setFormData({
                first_name: intern.first_name || '', last_name: intern.last_name || '', email: intern.email || '',
                mobile: intern.mobile || '', gender: intern.gender || '',
                date_of_birth: intern.date_of_birth ? new Date(intern.date_of_birth).toISOString().split('T')[0] : '',
                joining_date: intern.joining_date ? new Date(intern.joining_date).toISOString().split('T')[0] : '',
                expected_completion_date: intern.expected_completion_date ? new Date(intern.expected_completion_date).toISOString().split('T')[0] : '',
                department_id: intern.department_id || '', mentor_employee_id: intern.mentor_employee_id || '',
                training_batch: intern.training_batch || '', education: intern.education || '',
                institute: intern.institute || '', specialization: intern.specialization || '',
                status: intern.status || 'Applied', remarks: intern.remarks || '', image_url: intern.image_url || ''
            });
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingIntern) {
                await updateInternApi(editingIntern.intern_id, formData);
                toast.success('Intern updated successfully');
            } else {
                await createInternApi(formData);
    ...
    const handleConvert = async (e) => {
        e.preventDefault();
        try {
            await convertToEmployeeApi(convertingIntern.intern_id, convertData);
            toast.success('Intern successfully converted to Employee!');
    ...
    const handleAssignTraining = async (e) => {
        e.preventDefault();
        try {
            await assignLmsToInternApi(assigningIntern.intern_id, assignData);
    ...
    const columns = [
        { key: 'intern_code', label: 'Intern ID', render: (row) => <span className="font-bold text-[var(--accent)]">{row.intern_code}</span> },
        {
        ...
        },
        {
            key: 'actions', label: 'Actions', render: (row) => (
                <div className="flex items-center gap-1">
                    <button onClick={() => navigate(`/hr/intern/${row.intern_id}`)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg" title="View Profile">
                        <Eye size={16} />
                    </button>
                    {hasPermission('hr', 'edit', 'intern') && (
                        <button onClick={() => handleOpenModal(row)} className="p-1.5 text-emerald-500 hover:bg-emerald-50 rounded-lg" title="Edit">
                            <Edit size={16} />
                        </button>
                    )}
                    {hasPermission('hr', 'edit', 'intern') && (
                        <button onClick={() => { setAssigningIntern(row); setAssignData({ module_id: '', due_date: '' }); setIsAssignModalOpen(true); }} className="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded-lg" title="Assign LMS Training">
                            <BookOpen size={16} />
                        </button>
                    )}
                    ...
                    {hasPermission('hr', 'delete', 'intern') && (
                        <button onClick={() => handleDelete(row.intern_id)} className="p-1.5 text-rose-500 hover:bg-rose-50 rounded-lg" title="Delete">
                            <Trash2 size={16} />
                        </button>
                    )}
                </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-[1600px] mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 mt-4">
                <div className="flex items-center gap-5">
                    ...
                    <h1 className="text-2xl md:text-3xl font-black text-[var(--text-main)]">Intern Management</h1>
                </div>
                {hasPermission('hr', 'create', 'intern') && (
                    <button onClick={() => navigate('/hr/intern/new')} className="btn-primary flex items-center gap-2 px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                {[
                    { label: 'Total Interns', value: stats.total_interns || 0, icon: Users, color: 'text-blue-500', bg: 'bg-blue-50' },
                    { label: 'Active Interns', value: stats.active_interns || 0, icon: GraduationCap, color: 'text-amber-500', bg: 'bg-amber-50' },
            ...
            {/* Filters */}
            <div className="workspace-card p-2 flex flex-col md:flex-row gap-4 items-center bg-[var(--bg-card)] border border-[var(--border-color)] mb-6">
            ...
            {isLoading ? (
                ...
                <DataTable striped={true} columns={columns} data={paginatedInterns} loading={isLoading} totalCount={interns.length} filteredCount={filteredInterns.length} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
            ) : (
                <div className="flex flex-col gap-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 mb-6">
                        {paginatedInterns.length > 0 ? paginatedInterns.map((intern, index) => {
                            const defaultAvatarUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(intern.first_name + ' ' + intern.last_name)}&backgroundColor=3d6a7d,0f172a&textColor=ffffff`;
                            const avatarUrl = intern.image_url || defaultAvatarUrl;
                            ...
                            return (
                                ...
                                >
                                    <div onClick={() => navigate(`/hr/intern/${intern.intern_id}`)} className="relative py-6 w-full overflow-hidden bg-[var(--bg-workspace)] border-b border-[var(--border-color)] flex flex-col items-center justify-center cursor-pointer group/img">
                                        ...
                                        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); navigate(`/hr/intern/${intern.intern_id}`); }}
                                    ...
                                    <div className="p-4 flex-1 flex flex-col">
                                        <div className="flex-1 space-y-2">
                                            ...
                                            <div className="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--text-muted)] justify-center truncate">
                                                ...
                                                <span className="truncate">{intern.intern_code}</span>
                                            </div>
                                            {intern.department_name && (
                                                <div className="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--text-muted)] justify-center truncate">
                                                    <Building size={12} className="shrink-0" />
                                                    <span className="truncate">{intern.department_name}</span>
                                                </div>
                                            )}
                                        ...
                                        <div className="flex items-center justify-between pt-3 mt-3 border-t border-[var(--border-color)]">
                                            <div className="flex items-center gap-1">
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setAssigningIntern(intern); setAssignData({ module_id: '', due_date: '' }); setIsAssignModalOpen(true); }}
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleOpenModal(intern); }}
                                                ...
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleDelete(intern.intern_id); }}
                            ...
                            <div className="col-span-full py-16 text-center bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl border-dashed">
                                ...
                                <h3 className="text-lg font-bold text-[var(--text-main)]">No interns found</h3>
                                <p className="text-sm font-medium text-[var(--text-muted)] mt-1">Adjust your filters or add a intern.</p>
                            </div>
                        )}
                    </div>
                    {totalPages > 1 && (
            ...
            {/* Add/Edit Modal */}
            <Modal isOpen={isAddEditModalOpen} onClose={() => setIsAddEditModalOpen(false)} title={editingIntern ? "Edit Intern" : "Add Intern"} size="4xl">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button type="submit" className="btn-primary px-6">{editingIntern ? 'Save Changes' : 'Create Intern'}</button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal isOpen={isConvertModalOpen} onClose={() => setIsConvertModalOpen(false)} title="Convert to Employee" size="xl">
                <form onSubmit={handleConvert} className="space-y-4">
                    <p className="text-sm text-[var(--text-muted)] font-medium mb-4">You are about to convert <span className="font-bold text-[var(--text-main)]">{convertingIntern?.first_name} {convertingIntern?.last_name}</span> into a full-time employee.</p>
                    ...
                    <div>
                        ...
                        <select required value={convertData.designation_id} onChange={(e) => setConvertData({ ...convertData, designation_id: e.target.value })} className="w-full px-3 py-2 border rounded-xl bg-[var(--bg-workspace)]">
                            ...
                            {metadata.designations.filter(d => d.department_id === convertingIntern?.department_id).map(d => <option key={d.designation_id} value={d.designation_id}>{d.name}</option>)}
                        </select>
                    </div>
