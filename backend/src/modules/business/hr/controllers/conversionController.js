...
const approveConversion = async (req, res) => {
    try {
        ...
        await db.withTransaction(async (client) => {
            const requestRes = await client.query('SELECT * FROM hr_conversion_requests WHERE request_id = $1', [id]);
            ...
            if (request.intern_id && request.target_role === 'Trainee') {
                const internRes = await client.query('SELECT * FROM hr_interns WHERE intern_id = $1', [request.intern_id]);
                ...
                await client.query("UPDATE hr_interns SET status = 'Converted to Trainee' WHERE intern_id = $1", [request.intern_id]);
            ...
            // Handle Intern -> Employee OR Trainee -> Employee
                ...
                const sourceRes = await client.query(`SELECT * FROM ${sourceTable} WHERE ${idColumn} = $1`, [sourceId]);
                ...
                const userQuery = `
                    ...
                    RETURNING user_id
                `;
                const userRes = await client.query(userQuery, [name, sourceRecord.email, hashedPassword]);
                ...
                // 3. Update Status
                await client.query(`UPDATE ${sourceTable} SET status = 'Converted to Employee', updated_at = CURRENT_TIMESTAMP WHERE ${idColumn} = $1`, [sourceId]);

                // 4. (Optional) Transfer LMS assignments
                if (request.trainee_id) {
                    await client.query("UPDATE hr_lms_assignments SET employee_id = $1 WHERE trainee_id = $2", [new_employee_id, sourceId]);
                } else if (request.intern_id) {
                    await client.query("UPDATE hr_lms_assignments SET employee_id = $1 WHERE intern_id = $2", [new_employee_id, sourceId]);
                }
        ...
        } else if (error.message.startsWith('Request is already')) {
